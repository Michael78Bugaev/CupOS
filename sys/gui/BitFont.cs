...
namespace Cup.sys.gui
{
    ...
    static class BitFont
    {
        ...
        public static void DrawBitFontString(string FontName, Color color, string Text, int X, int Y, int Devide = 2, bool DisableAntiAliasing = false)
        {
            ...
            for (int l = 0; l < Lines.Length; l++)
            {
                ...
                {
                    char c = Lines[l][i];
                    UsedX += DrawBitFontChar(GUI.canvas, bitFontDescriptor.MS, bitFontDescriptor.Size, color, bitFontDescriptor.Charset.Impl_Str_IndexOf(c), UsedX + X, Y + bitFontDescriptor.Size * l, !DisableAntiAliasing) + Devide;
                }
            }
        }
